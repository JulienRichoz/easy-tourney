  ...
  <select v-model="filterDate" class="ml-4 p-2 border border-gray-300 rounded-md">
    ...
    >
      ...
      <template #footer>
        ...
        <ButtonComponent variant="primary" @click="saveTourney" :disabled="nameError">{{
...
export default {
  ...
  computed: {
    // Les filtres "filterStatus" et "filterDate" permettent de filtrer dynamiquement les tournois affichés.
    // En utilisant un computed property "filteredTourneys", la liste des tournois est recalculée automatiquement chaque fois
    // que les valeurs des filtres changent, offrant une expérience réactive et fluide. (Aide avec chatgpt)
    filteredTourneys() {
  ...
  methods: {
    ...
    isNameUnique(name) {
      return !this.tourneys.some(
        (tourney) => tourney.name.toLowerCase() === name.toLowerCase()
      );
    },
  },
  mounted() {
