  ...
  try {
    ...
    if (!['player', 'assistant'].includes(type)) {
      return res
        ...
        .json({
          message:
            'Type d\'équipe invalide. Doit être "player" ou "assistant".',
        });
    }
...
exports.getTeamById = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .json({
        message:
          'Erreur serveur lors de la récupération des détails de l\'équipe.',
      });
  }
};
...
exports.updateTeam = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: 'Erreur serveur lors de la mise à jour de l\'équipe.' });
  }
};
  ...
  try {
    ...
    if (!isAdmin) {
      ...
      if (registrationStatus !== 'active') {
        return res
          .status(403)
          .json({ message: 'Les inscriptions ne sont pas ouvertes.' });
      }
    }
    ...
    if (userTourney.teamId && userTourney.teamId !== teamId) {
      return res
        .status(400)
        .json({
          message: 'L\'utilisateur est déjà assigné à une autre équipe.',
        });
    }
    ...
    console.error(
      'Erreur lors de l\'assignation de l\'utilisateur à l\'équipe :',
      error
    );
    res
      .status(500)
      .json({
        message:
          'Erreur serveur lors de l\'assignation de l\'utilisateur à l\'équipe.',
      });
  }
};
...
exports.removeUserFromTeam = async (req, res) => {
  ...
  try {
    ...
    if (!userTourney) {
      return res
        .status(404)
        .json({ message: 'Association utilisateur-équipes introuvable.' });
    }
    ...
    res
      .status(200)
      .json({
        message: 'Utilisateur supprimé de l\'équipe avec succès.',
        userTourney,
      });
  } catch (error) {
    console.error(
    ...
    res
      ...
      .json({
        message:
          'Erreur serveur lors de la suppression de l\'utilisateur de l\'équipe.',
      });
  }
};
...
exports.generateTeams = async (req, res) => {
  ...
  try {
    ...
    if (existingTeams.length >= playerTeamCount + 1) {
      return res
        .status(400)
        .json({ message: 'Le nombre maximal d\'équipes a déjà été atteint.' });
    }

    // Vérifier si une équipe d'assistant existe déjà
    const assistantTeamExists = existingTeams.some(
      (team) => team.type === 'assistant'
    );

    // Compter le nombre actuel d'équipes de joueurs
    const currentPlayerTeams = existingTeams.filter(
      (team) => team.type === 'player'
    ).length;
    ...
    if (teamsToGenerate <= 0) {
      return res
        .status(400)
        .json({ message: 'Toutes les équipes de joueurs sont déjà générées.' });
    }
    ...
    res
      .status(201)
      .json({ message: 'Équipes générées avec succès', teams: createdTeams });
  } catch (error) {
...
exports.resetTeamsAndReassignUsers = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .json({
        message:
          'Équipes supprimées et utilisateurs réassignés en tant que Guest.',
      });
  } catch (error) {
    console.error(
      'Erreur lors de la réinitialisation des équipes et des utilisateurs:',
      error
    );
    res
      .status(500)
      .json({
        message:
          'Erreur serveur lors de la réinitialisation des équipes et des utilisateurs.',
      });
  }
};
...
exports.autoFillTeams = async (req, res) => {
  ...
  try {
    for (const assignment of assignments) {
      ...
      if (!userTourney) {
        throw new Error(
          `L'utilisateur ${userId} ne participe pas au tournoi ${tourneyId}.`
        );
      }
...
exports.getUnassignedTeams = async (req, res) => {
  ...
  try {
    ...
    console.error(
      'Erreur lors de la récupération des équipes non assignées :',
      error
    );
    res
      .status(500)
      .json({
        message: 'Erreur lors de la récupération des équipes non assignées.',
        error,
      });
  }
};
