...
const { Tourney, Field, SportsFields, Sport, TeamSetup, ScheduleTourney, User, Team, UsersTourneys, Role, InviteToken, Pool, PoolSchedule, Game } = require('../models');
...
exports.getTourneyStatuses = async (req, res) => {
    try {
        ...
        const { name, tourneyType, status, fieldAssignmentStatus, sportAssignmentStatus, registrationStatus, poolStatus, planningStatus } = tourney;
        res.status(200).json({
            name,
            tourneyType,
            status,
...
exports.getTourneyPlanningDetails = async (req, res) => {
    try {
      const { tourneyId } = req.params;
