...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedTypes.includes(file.mimetype)) {
      ...
      cb(
        new Error(
          'Format de fichier non supporté. Veuillez télécharger une image au format JPG, PNG, GIF ou WEBP.'
        ),
        false
      );
    }
...
exports.createSport = async (req, res) => {
  try {
    ...
    if (!name || !rule) {
      return res
        .status(400)
        .json({ message: 'Les champs \'name\' et \'rule\' sont requis.' });
    }
...
exports.getSportById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Erreur lors de la récupération du sport' });
  }
};
