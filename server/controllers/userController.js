    ...
    if (Array.isArray(tourneyIds) && tourneyIds.length > 0) {
      ...
      if (existingTourneys.length !== tourneyIds.length) {
        ...
        return res
          .status(400)
          .json({
            message: 'Un ou plusieurs tournois fournis sont invalides.',
          });
      }
    }
...
exports.addUserToTourney = async (req, res) => {
  ...
  try {
    ...
    if (existingAssociation) {
      return res
        .status(400)
        .json({ message: 'L\'utilisateur est déjà inscrit à ce tournoi.' });
    }
    ...
    res
      .status(200)
      .json({ message: 'Utilisateur ajouté au tournoi avec succès.' });
  } catch (error) {
    console.error(
      'Erreur lors de l\'ajout de l\'utilisateur au tournoi :',
      error
    );
    res.status(500).json({ message: 'Erreur serveur' });
  }
...
exports.removeUserFromTourney = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: 'Utilisateur retiré du tournoi avec succès.' });
  } catch (error) {
    console.error(
      'Erreur lors du retrait de l\'utilisateur du tournoi :',
      error
    );
    res.status(500).json({ message: 'Erreur serveur' });
  }
