...
const verifyFieldAssignmentStatus = async (req, res, next) => {
    ...
    try {
        ...
        if (tourney.fieldAssignmentStatus === 'completed') {
            return res.status(403).json({ message: 'L\'action requiert que la page Terrain soit en mode édition.' });
        }

        next(); // Passer à la prochaine étape si le statut est correct
    } catch (error) {
        console.error('Erreur lors de la vérification du fieldAssignmentStatus:', error);
        res.status(500).json({ message: 'Erreur serveur.' });
    }
};
...
const verifySportAssignmentStatus = async (req, res, next) => {
    ...
    try {
        ...
        if (tourney.sportAssignmentStatus === 'completed') {
            return res.status(403).json({ message: 'L\'action requiert que la page Assignation sport-terrain soit en mode édition.' });
        }

        next(); // Passer à la prochaine étape si le statut est correct
    } catch (error) {
        console.error('Erreur lors de la vérification du sportAssignmentStatus:', error);
        res.status(500).json({ message: 'Erreur serveur.' });
    }
};
...
const verifyPlanningStatus = async (req, res, next) => {
    ...
    try {
        ...
        if (tourney.planningStatus === 'completed') {
            return res.status(403).json({ message: 'L\'action requiert que la page Planning soit en mode édition.' });
        }
